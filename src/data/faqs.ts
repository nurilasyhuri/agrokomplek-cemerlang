export interface FaqItem {
  question: string;
  answer: string;
}

export const HOME_FAQS: FaqItem[] = [
  {
    question: 'Berapa biaya pembuatan greenhouse per meter?',
    answer: 'Biaya tidak bisa ditentukan hanya berdasarkan luas bangunan. Jenis rangka (bambu, baja ringan, pipa galvanis), ketebalan plastik UV, kerapatan insect net, ventilasi, kedalaman fondasi, dan sistem irigasi menentukan total biaya. Gunakan kalkulator RAB online kami untuk estimasi awal atau konsultasikan ukuran lahan via WhatsApp.',
  },
  {
    question: 'Apakah melayani survei lahan dan instalasi greenhouse ke luar Jawa Timur?',
    answer: 'Ya. Kami berbasis di Malang dan melayani pengiriman material sistem knock-down, supervisi teknis, hingga instalasi penuh oleh tim teknisi berpengalaman ke seluruh Indonesia (Jawa Timur, Jawa Tengah, Jawa Barat, Bali, Sumatera, Kalimantan, Sulawesi, hingga Papua).',
  },
  {
    question: 'Apa perbedaan plastik UV greenhouse dan plastik biasa?',
    answer: 'Plastik UV greenhouse mengandung aditif UV stabilizer (14%-20%) dengan ketebalan standar 200 mikron (0.20 mm). Berfungsi menyaring sinar ultraviolet berlebih, menahan benturan angin/hujan, anti-drip (mencegah tetesan kondensasi jamur), dan bertahan 3-5 tahun di bawah iklim tropis.',
  },
  {
    question: 'Berapa lama garansi dan daya tahan struktur greenhouse yang dibangun?',
    answer: 'Daya tahan bergantung pada tipe struktur yang dipilih: Greenhouse pipa galvanis hot-dip bertahan 10-15+ tahun, baja ringan 7-10 tahun, dan bambu awet 3-5 tahun. Plastik UV 200 mikron yang kami pasang memiliki ketahanan paparan sinar ultraviolet 3 hingga 5 tahun.',
  },
  {
    question: 'Bagaimana alur dan proses pembuatan greenhouse?',
    answer: 'Proses mencakup: 1) Konsultasi kebutuhan dan jenis tanaman, 2) Pengumpulan informasi ukuran lahan, 3) Pemilihan tipe rangka dan spesifikasi penutup, 4) Penyusunan RAB transparan, 5) Survei lokasi, 6) Fabrikasi material di workshop Malang, 7) Pengiriman dan perakitan di lokasi, 8) Uji kekokohan dan serah terima kunci.',
  },
  {
    question: 'Apakah bisa konsultasi teknis terlebih dahulu sebelum membuat proyek?',
    answer: 'Tentu. Anda dapat berkonsultasi secara gratis dengan tim teknis kami mengenai estimasi kebutuhan material, arah angin, pemilihan ventilasi monitor roof, atau perhitungan kebutuhan selang drip irigasi melalui WhatsApp.',
  },
];

export const GREENHOUSE_FAQS: FaqItem[] = [
  {
    question: 'Berapa perkiraan biaya pembuatan greenhouse per m² di Indonesia?',
    answer: 'Kisaran biaya pembuatan greenhouse umumnya terbagi berdasarkan tipe rangka: Greenhouse Bambu (Rp 100.000 - Rp 175.000/m²), Greenhouse Baja Ringan (Rp 200.000 - Rp 350.000/m²), dan Greenhouse Pipa Galvanis Hot-Dip komersial tahan 10-15 tahun (Rp 350.000 - Rp 650.000+/m²). Biaya akhir menyesuaikan jenis ventilasi, komoditas, dan kelengkapan irigasi.',
  },
  {
    question: 'Apa saja tipe konstruksi greenhouse yang dapat dibangun?',
    answer: 'Agrokomplek Cemerlang melayani pembuatan: 1) Greenhouse Pipa Galvanis Hot-Dip anti-karat untuk hortikultura modern (melon, tomat, paprika), 2) Greenhouse Baja Ringan berbobot ringan dan presisi, 3) Greenhouse Rangka Bambu ekonomis untuk pembibitan/sayuran, dan 4) Tunnel Garam (Prisma Garam) spesialis pesisir tahan korosi air laut.',
  },
  {
    question: 'Apakah Agrokomplek Cemerlang melayani pengerjaan di luar Kota Malang dan luar Pulau Jawa?',
    answer: 'Ya. Kami berbasis di Kedungkandang, Kota Malang dan melayani fabrikasi serta pengiriman material sistem knock-down maupun pengerjaan instalasi tim teknisi ke seluruh Jawa Timur, Jawa Tengah, Jawa Barat, Bali, Sumatera, Kalimantan, Sulawesi, hingga kawasan timur Indonesia.',
  },
  {
    question: 'Berapa lama waktu pembuatan dan perakitan greenhouse di lokasi?',
    answer: 'Untuk greenhouse skala menengah (luas 100 m² - 500 m²), proses fabrikasi dan perakitan lapangan membutuhkan waktu rata-rata 10 hingga 21 hari kerja, tergantung kondisi topografi lahan dan kondisi cuaca di lokasi proyek.',
  },
  {
    question: 'Apakah biaya pembuatan sudah termasuk sistem irigasi tetes (drip) dan fertigasi?',
    answer: 'Kami menyediakan opsi konstruksi bangunan saja (rangka, atap plastik UV, dinding insect net) maupun paket full-set turnkey yang sudah dilengkapi sistem irigasi tetes (drip irrigation 16mm), pipa fertigasi, dan meja tanam/gully hidroponik siap tanam.',
  },
  {
    question: 'Bagaimana sistem pembayaran dan tahapan kontrak kerja sama jasa pembuatan greenhouse?',
    answer: 'Sistem pembayaran dilakukan secara bertahap (termin) sesuai progres kerja: DP pemesanan & persiapan material (termin 1), mobilisasi material ke lokasi (termin 2), dan pelunasan setelah perakitan selesai serta serah terima kunci (termin 3). Seluruh termin dituangkan dalam surat perjanjian kerja yang jelas.',
  },
];
